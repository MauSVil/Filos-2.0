...
const Contacts = (props: ContactsProps) => {
  ...
  const orderedContacts = useMemo(() => {
    return Object.values(contacts).sort((a, b) => {
      const dateA = moment(a.lastMessageSent || 0)
        .tz("America/Mexico_City")
        .toDate()
        .getTime();
      const dateB = moment(b.lastMessageSent || 0)
        .tz("America/Mexico_City")
        .toDate()
        .getTime();
          ...
          {orderedContacts.map((contact) => {
            return (
              <div
                ...
                onClick={() =>
                  handleSelectionChange(new Set([contact.phone_id]))
                }
              >
                <div className="w-3 overflow-hidden inline-block absolute bottom-0 right-0">
                  <div
                    className={cn(
                      "h-5 rotate-45 transform origin-bottom-left",
                      {
                ...
                <div className="grid gap-1">
                  <p className="text-sm font-medium leading-none">
                    {contact.fullName || contact.phone_id}
                  </p>
                  <p className="text-xs text-muted-foreground mb-2">
                    {moment(contact.lastMessageSent)
                      .tz("America/Mexico_City")
                      .fromNow()}
                  </p>
                  <div className="flex items-center gap-1 mb-2">
                    {contact.newMessage && <Badge variant={"new"}>Nuevo</Badge>}
                    {contact.aiEnabled && <Badge variant={"IA"}>IA</Badge>}
                  </div>
                </div>
                {selectedChat === contact.phone_id && <Check size={15} />}
              </div>
            );
          })}
        </ScrollArea>
