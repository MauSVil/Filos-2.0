...
const NewCatalogComponent = (props: Props) => {
  ...
  const handleCreateClick = async () => {
    setLoading(true);
    toast.promise(ky.post('/api/catalogs/new', { json: { pdfIds: props.productIds, name: value } }), {
  ...
  return (
    <AlertDialog open={isOpen} onOpenChange={(open) => !open}>
      <AlertDialogContent>
        ...
        <Input placeholder="Escriba un nombre para el catalogo..." value={value} onChange={(e) => setValue(e.target.value)} />
        <AlertDialogFooter className="gap-2">
          <AlertDialogAction asChild>
            <Button onClick={handleCloseClick}>Cancelar</Button>
          </AlertDialogAction>
          <AlertDialogAction asChild>
            <Button disabled={loading} variant="outline" onClick={handleCreateClick}>
