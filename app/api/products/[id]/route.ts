...
import { ProductInput, ProductInputModel } from "@/types/RepositoryTypes/Product";
...
export const PUT = async (req: NextRequest, params: { params: Promise<{ id: string }> }) => {
  ...
  try {
    ...
    if (error instanceof Error) {
      return NextResponse.json({ error: 'Ha ocurrido un problema P.1', devError: error.message }, { status: 400 });
    }
  ...
  try {
    ...
    if (error instanceof Error) {
      return NextResponse.json({ error: 'Ha ocurrido un problema P.2', devError: error.message }, { status: 400 });
    }
    return NextResponse.json({ error: "Internal Server Error" }, { status: 500 });
  }
