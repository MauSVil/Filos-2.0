...
import {
  ...
  OrderRepositoryFilterModel,
  OrderUpdateInputModel,
} from "@/types/RepositoryTypes/Order";
...
export class OrdersRepository {
  static async findOne(
    ...
    const order = await db.collection("orders").findOne<Order>({
      ...
      ...(dateRange
        ? { requestDate: { $gte: dateRange.from, $lte: dateRange.to } }
        : {}),
    });
  ...
  static async find(filter: OrderRepositoryFilter = {}): Promise<Order[]> {
    await init();
    const filters = await OrderRepositoryFilterModel.parse(filter);
    const { page, dateRange, dueDateRange, orConditions, ids, ...rest } =
      filters;
  ...
  static async updateOne(id: string, order: Partial<OrderInput>) {
    ...
    await db
      .collection("orders")
      .updateOne({ _id: new ObjectId(id) }, { $set: orderParsed });
    await HistoryMovementsRepository.insertOne({
      values: orderParsed,
      type: "update",
      collection: "orders",
